...
def XGBoost(train_x, train_y, test_x):
    ...
    for i, (train_index, verify_index) in enumerate(kf.split(train_x, train_y)):
        ...
        if 1:
            #param = {'max_depth':10, 'eta':0.5, 'verbosity':1, 'objective':'multi:softprob'}
            model = XGBClassifier(max_depth=5,learning_rate= 0.5, verbosity=1, objective='multi:softprob',random_state=1)
...
y_test, f1_mean = XGBoost(train_x, train_y, test_x)
a = 1